<template>
    ...
    <div class="container">
        <h4 class="mt-5 mb-5 ml-5">State Nodal Officers/Department Nodal officer</h4>
        <h3>{{ departmentName }}</h3>
         
    <Link href="/admin/nodal/create" class="p-2 m-5 bg-blue-5 text-white shadow-2">New Department Nodal</Link>
        <div class="q-pa-md">
            <q-table
            :rows="stateNodalOfficers"
            ...
            >
            <template v-slot:top>
                <span class="text-2xl">Department Nodal Officer List</span>
                <q-space/>
            ...
            <template v-slot:body-cell-roles="props">
                <q-td :props="props" >
                    <div v-if="props.row.role_id==3">
                        <!-- <DepartmentNodalLabel :value=props.row.role.name   /> -->
                        {{ props.row.role.name }}
                    </div>
                    <div v-else-if="props.row.role_id==4">
                        <!-- <SubOrganizationLabel :value=props.row.role.name   /> -->
                        {{ props.row.role.name }}
                    </div>
                    <div v-else-if="props.row.role_id==6">
                        <!-- <DepartmentalAppellateLabel :value=props.row.role.name   /> -->
                        {{ props.row.role.name }}
                    </div>
                    <div v-else>
                        {{ props.row.role.name }}
            ...
            <template v-slot:body-cell-more="props">
                <q-td :props="props">
                    ...
                    <div v-else>
                        <q-btn
                            unelevated
                            @click="moreClick(props.row)"
                            rounded
                            color="blue-4"
                            size='sm'
                            label="Click to go to sub"
                        />
                    </div>
                </q-td>
            </template>
                ...
                <template v-slot:body-cell-actions="props">
                    ...
                    :props="props">
                        ...
                        <q-btn v-if="props.row.role_id!=2 "
                            dense
                            round
                            flat
                            color="red"
...
const columns =  [
    ...
    {name:'more', align:'left', label:'Sub Org', field:'id'},
